...
export default function App() {
  ...
  useEffect(() => {
    AdMob.initialize({
      // testingDevices: ['YOUR_TEST_DEVICE_ID'], // Buraya logcat'ten aldığınız cihaz kimliğini ekleyin
  ...
  const handleWatchAd = async () => {
    ...
    try {
      const options: RewardAdOptions = {
        adId: 'ca-app-pub-3940256099942544/5354046379', // Ödüllü Geçiş Reklamı (Rewarded Interstitial) Test ID
      };
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-slate-950 text-white select-none touch-none">
      ...
      {/* UI Overlay - Top */}
      <div className="absolute top-0 left-0 w-full p-4 md:p-6 flex flex-col gap-4 pointer-events-none safe-top z-[60]">
        <div className="w-full flex justify-between items-start pointer-events-none">
          {/* Left Side: Language & Zoom */}
          <div className="flex flex-col gap-2 pointer-events-auto">
            <button
              onClick={() => setLang(lang === 'en' ? 'tr' : 'en')}
              className="bg-slate-900/90 backdrop-blur-md border border-slate-800 rounded-xl p-3 flex items-center gap-2 hover:bg-slate-800 active:scale-95 transition-all shadow-lg"
            >
            ...
            <div className="flex flex-col gap-1">
              <button
                onClick={() => scaleRef.current = Math.min(3000, scaleRef.current + 150)}
                className="bg-slate-900/90 backdrop-blur-md border border-slate-800 rounded-xl p-3 hover:bg-slate-800 active:scale-95 transition-all shadow-lg"
              >
                <ZoomIn size={18} className="text-slate-400" />
              </button>
              <button
                onClick={() => scaleRef.current = Math.max(150, scaleRef.current - 150)}
                className="bg-slate-900/90 backdrop-blur-md border border-slate-800 rounded-xl p-3 hover:bg-slate-800 active:scale-95 transition-all shadow-lg"
              >
          ...
          <div className="flex flex-wrap justify-center gap-2 md:gap-3 pointer-events-auto max-w-[50%] md:max-w-none">
            {nickname && (
              <div className="bg-slate-900/90 backdrop-blur-md border border-slate-800 rounded-xl px-3 py-2 flex items-center gap-2 shadow-lg">
                <User size={16} className="text-purple-400" />
                <span className="font-mono text-xs font-bold truncate max-w-[70px]">{nickname}</span>
              </div>
            )}
            <div className="bg-slate-900/90 backdrop-blur-md border border-slate-800 rounded-xl px-3 py-2 flex items-center gap-2 shadow-lg">
              <RotateCcw size={16} className="text-blue-400" />
              <span className="font-mono text-sm font-bold">{formatTime(gameTime)}</span>
            </div>
            <div className="bg-slate-900/90 backdrop-blur-md border border-slate-800 rounded-xl px-3 py-2 flex items-center gap-2 shadow-lg">
            ...
            <div className="bg-slate-900/90 backdrop-blur-md border border-slate-800 rounded-xl px-3 py-2 flex items-center gap-2 shadow-lg">
              ...
              <span className="font-mono text-sm font-bold">{solvedCount}</span>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2 pointer-events-auto">
            <button
              ...
              disabled={jokers.double.used || !selectedCountry || !!feedback || !!isJokerLoading}
              className={cn(
                "bg-slate-900/90 backdrop-blur-md border border-slate-800 rounded-xl p-3 flex flex-col items-center gap-1 transition-all shadow-lg",
                (jokers.double.used || !selectedCountry || !!feedback || !!isJokerLoading) ? "opacity-30 grayscale cursor-not-allowed" : "hover:bg-slate-800 active:scale-95",
                jokers.double.active && "border-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.6)]"
              )}
            >
              <RotateCcw size={20} className="text-blue-400" />
              <span className="text-[8px] font-black uppercase">x2</span>
            </button>
            <button
              onClick={useFortyJoker}
              disabled={jokers.forty.used || !selectedCountry || !!feedback || !!isJokerLoading}
              className={cn(
                "bg-slate-900/90 backdrop-blur-md border border-slate-800 rounded-xl p-3 flex flex-col items-center gap-1 transition-all shadow-lg",
                (jokers.forty.used || !selectedCountry || !!feedback || !!isJokerLoading) ? "opacity-30 grayscale cursor-not-allowed" : "hover:bg-slate-800 active:scale-95"
              )}
            >
              <Percent size={20} className="text-emerald-400" />
              <span className="text-[8px] font-black uppercase">40%</span>
            </button>
            <button
              onClick={usePhoneJoker}
              disabled={jokers.phone.used || !selectedCountry || !!feedback || !!isJokerLoading}
              className={cn(
                "bg-slate-900/90 backdrop-blur-md border border-slate-800 rounded-xl p-3 flex flex-col items-center gap-1 transition-all shadow-lg",
                (jokers.phone.used || !selectedCountry || !!feedback || !!isJokerLoading) ? "opacity-30 grayscale cursor-not-allowed" : "hover:bg-slate-800 active:scale-95"
              )}
            >
              <Phone size={20} className="text-purple-400" />
              <span className="text-[8px] font-black uppercase">TEL</span>
            </button>
            <button
              onClick={useAudienceJoker}
              disabled={jokers.audience.used || !selectedCountry || !!feedback || !!isJokerLoading}
              className={cn(
                "bg-slate-900/90 backdrop-blur-md border border-slate-800 rounded-xl p-3 flex flex-col items-center gap-1 transition-all shadow-lg",
                (jokers.audience.used || !selectedCountry || !!feedback || !!isJokerLoading) ? "opacity-30 grayscale cursor-not-allowed" : "hover:bg-slate-800 active:scale-95"
              )}
            >
              <Users size={20} className="text-yellow-400" />
              <span className="text-[8px] font-black uppercase">AUD</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <AnimatePresence>
        {selectedCountry && quizOptions.length > 0 && (
          <motion.div
            ...
            className="absolute bottom-0 left-0 w-full z-50 pointer-events-auto safe-bottom"
          >
            <div className="bg-slate-900/95 backdrop-blur-2xl border-t border-slate-800 rounded-t-[2.5rem] p-6 md:p-8 shadow-[0_-20px_50px_rgba(0,0,0,0.5)] max-w-2xl mx-auto">
              <div className="w-12 h-1.5 bg-slate-800 rounded-full mx-auto mb-6" />
              
              <div className="mb-6 text-center">
                <p className="text-[10px] uppercase font-black tracking-widest text-blue-500 mb-1">{t.findCountry}</p>
              ...
              {jokers.audience.active && (
                <div className="mb-6 grid grid-cols-5 gap-2 h-24 items-end px-4">
                  {quizOptions.map((opt, i) => (
                    <div key={opt.id} className="flex flex-col items-center gap-1">
                      ...
                      />
                      <span className="text-[10px] font-black text-slate-500">{String.fromCharCode(65 + i)}</span>
                    </div>
                  ))}
                </div>
              )}
                ...
                {quizOptions.map((option, i) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={!!feedback}
                      className={cn(
                        "w-full py-4 px-6 rounded-2xl font-bold text-base md:text-lg transition-all flex items-center justify-between group",
                        !feedback && "bg-slate-800 hover:bg-slate-700 text-white active:scale-[0.98]",
                        feedback && isCorrect && "bg-emerald-600 text-white shadow-[0_0_20px_rgba(16,185,129,0.4)]",
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span className="truncate max-w-[180px] md:max-w-none">{getCountryName(option)}</span>
                      </div>
      ...
      <AnimatePresence>
        {isJokerLoading && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="absolute inset-0 bg-slate-950/80 backdrop-blur-md flex items-center justify-center z-[110]"
          >
            <motion.div 
              initial={{ scale: 0.9, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              className="bg-slate-900 border border-slate-800 p-10 rounded-[3rem] flex flex-col items-center gap-6 shadow-2xl"
            >
              <motion.div
                animate={{ rotate: 360 }}
                transition={{ duration: 2, repeat: Infinity, ease: "linear" }}
              ...
              >
                {isJokerLoading === 'phone' ? (
                  <Phone size={48} className="text-purple-400" />
                ) : (
      ...
      <AnimatePresence>
        {feedback && (
          ...
          >
            <div className={cn(
              "px-8 py-4 rounded-full flex items-center gap-4 shadow-2xl backdrop-blur-xl border-2",
